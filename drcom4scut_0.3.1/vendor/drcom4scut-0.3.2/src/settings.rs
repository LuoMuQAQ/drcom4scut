...
fn get_credential(matches: &ArgMatches, cfg: &Config, key: &str, env_key: &str) -> Option<String> {
    matches
...
impl Settings {
    ...
    fn resolve(mut settings: Settings, matches: &ArgMatches, cfg: Config) -> Settings {
        ...
        settings.username = get_credential(matches, &cfg, "username", "DRCOM_USERNAME")
            .expect("Username is REQUIRED!");
        settings.password = get_credential(matches, &cfg, "password", "DRCOM_PASSWORD")
